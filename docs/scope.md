...
1. **Add standard derived variables.** Variables that are generally useful and follow
   directly from the raw data — e.g. a BMI dummy from height and weight, or a single
   birth month reconciled across modules.
1. **Add derived variables relevant as GETTSIM inputs.** Variables shaped for
...
- **A fixed merged dataset.** It ships a helper (`create_final_dataset`) and a metadata
  catalogue; you assemble the variables and waves you need (see
