...
`hwealth.dta` holds household-level wealth; one of its variables, `p010ha`, is the
...
GETTSIM, which expects flat columns, and a flat layout keeps merges explicit. If you
want them as an index in your own analysis, set it yourself:
...
definition shows the raw SOEP variable(s) it is built from, which you can then look up
on paneldata.org. The [Variables](variables.md) reference lists every variable with its
