...
building final merged datasets. Built on **pytask** for task orchestration and **pixi**
...
1. `pixi run -e py314 prek run --all-files` (quality checks: linting, formatting, yaml,
   etc.)
...
- **final_dataset.py** -- `create_final_dataset(modules, variables, survey_years)`
  merges selected variables from multiple modules via outer join on index variables.
