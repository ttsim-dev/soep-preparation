...
- `data`: raw data in `.dta` format with directories for each version (e.g. `V41`)
...
**_Inside the directory `soep_preparation/data` fill the folder `V41` containing the raw
...
The SOEP data is available in different versions, the latest being version 41. This
project is currently set up to work with version 41. It is relevant to note that the
...
1. Add the raw data file to the data directory
   ...
   `soep_preparation/data/V41`). As an example, say you want to add the module
