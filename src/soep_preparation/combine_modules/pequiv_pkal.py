...
def combine(pequiv: pd.DataFrame, pkal: pd.DataFrame) -> pd.DataFrame:
    ...
    The earliest survey year in which a person either reports positive annual
    statutory pension income (`gesetzliche_rente_y`, pequiv) or any retirement
    month in the previous year (`number_of_months_in_retirement_last_year`, pkal).
    The consuming project turns it into a claiming age with the birth year.
