...
_MAPPED_SOEP_VARIABLES = sorted(
    {
        soep_variable
        for soep_variable in SOEP_TO_GETTSIM.values()
...
def task_write_mapping_report(
    ...
    """Write the GETTSIM-input mapping report (coverage counts plus the table).

    Args:
        out_path: Where to write the JSON report.
    """
    report = create_mapping_report(mapping=SOEP_TO_GETTSIM)
...
def task_build_gettsim_inputs(
    ...
    Merges the SOEP variables that `SOEP_TO_GETTSIM` maps to a GETTSIM input into a
    final dataset, renames them to their GETTSIM input qnames, and writes a flat
    `.arrow` file that GETTSIM consumes directly.
