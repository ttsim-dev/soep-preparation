...
def test_apply_lowest_float_dtype_assert_dtype():
    expected = pd.Series([0.1, 0.2], dtype="float[pyarrow]").dtype
    sr = pd.Series([0.1, 0.2])
    actual = apply_lowest_float_dtype(sr).dtype
    assert actual == expected


def test_apply_lowest_int_dtype_assert_dtype_int8():
    expected = pd.Series([-128, 127], dtype="int8[pyarrow]").dtype
    sr = pd.Series([-128, 127])
    actual = apply_lowest_int_dtype(sr).dtype
    assert actual == expected


def test_apply_lowest_int_dtype_assert_dtype_int16():
...
def test_apply_lowest_int_dtype_assert_dtype_uint8():
...
def test_apply_lowest_int_dtype_assert_dtype_uint16():
...
def test_find_lowest_int_dtype_assert_dtype():
    expected = pd.Series([0, 255], dtype="uint8[pyarrow]").dtype
    sr = pd.Series([0, 255])
    actual = find_lowest_int_dtype(sr)
    assert actual == expected
