...
def test_dependent_employment_income_sums_wage_and_bonus_components() -> None:
    ...
    actual = _calculate_dependent_employment_income(
        earnings_from_first_job=_make_series(30000.0),
        earnings_from_second_job=_make_series(2000.0),
        thirteenth_monthly_salary=_make_series(2500.0),
        fourteenth_monthly_salary=_make_series(0.0),
        christmas_bonus=_make_series(1000.0),
        holiday_bonus=_make_series(500.0),
        profit_sharing=_make_series(300.0),
        other_bonuses=_make_series(200.0),
    )
...
def test_dependent_employment_income_ignores_missing_components() -> None:
    ...
    actual = _calculate_dependent_employment_income(
        earnings_from_first_job=_make_series(30000.0),
        earnings_from_second_job=_make_series(None),
        thirteenth_monthly_salary=_make_series(None),
        fourteenth_monthly_salary=_make_series(None),
        christmas_bonus=_make_series(1000.0),
        holiday_bonus=_make_series(None),
        profit_sharing=_make_series(None),
        other_bonuses=_make_series(None),
    )
